...
!!! tip "Cron support"
    Full cron expression support requires `croniter` (`pip install croniter`). Without it, the scheduler uses a minimal built-in parser that handles simple `minute hour * * *` patterns only.
